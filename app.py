...
app.secret_key = os.environ.get('SECRET_KEY') or generate_password_hash(ADMIN_USER + '|' + ADMIN_PASS)[:64]
...
def db():
    if 'db' not in g:
        ...
        # Repair legacy partner accounts created before service was stored on accounts.
        g.db.execute("UPDATE accounts SET service=(SELECT service FROM partners WHERE partners.account_id=accounts.id) WHERE role='partner' AND (service IS NULL OR service='')")
...
def login():
    ...
    if request.method=='POST':
        ...
        a=q('''SELECT a.*, COALESCE(a.service,p.service) AS effective_service FROM accounts a LEFT JOIN partners p ON p.account_id=a.id WHERE a.active=1 AND (lower(a.username)=? OR lower(a.name)=? OR replace(a.phone," ","")=?) ORDER BY a.id LIMIT 1''',(identifier,identifier,identifier.replace(' ','')),True)
        if a and check_password_hash(a['password_hash'],pw):
            session['account_id']=a['id']
            return redirect(SERVICE_PATHS.get(a['effective_service'] or a['service'],'/services') if a['role']=='partner' else (next_url if next_url.startswith('/') else '/services'))
...
def provider_entry():
    ...
    if a and a['role']=='partner':
        ps=q('SELECT service FROM partners WHERE account_id=?',(a['id'],),True)
        pservice=(ps['service'] if ps else a['service'])
        if pservice!=service: return redirect(SERVICE_PATHS.get(pservice,'/services'))
...
def admin_partners():
    if request.method=='POST':
        ...
        if name and username and pw and service in SERVICES and not q('SELECT id FROM accounts WHERE username=?',(username,),True):
            cur=db().execute('INSERT INTO accounts(name,username,password_hash,role,service,phone,created_at) VALUES(?,?,?,?,?,?,?)',(name,username,generate_password_hash(pw),'partner',service,request.form.get('phone',''),now()))
...
@app.post(ADMIN_PATH+'/partner/<int:pid>/delete')
@admin_required
def delete_partner(pid):
    p=q('SELECT * FROM partners WHERE id=?',(pid,),True)
    if not p: abort(404)
    aid=p['account_id']
    # Keep request history readable while removing the active partner/account.
    db().execute('UPDATE requests SET partner_id=NULL WHERE partner_id=?',(pid,))
    db().execute('UPDATE ratings SET partner_id=NULL WHERE partner_id=?',(pid,))
    db().execute('DELETE FROM partners WHERE id=?',(pid,))
    db().execute('DELETE FROM accounts WHERE id=? AND role=\'partner\'',(aid,))
    return redirect(request.referrer or url_for('admin_partners'))
